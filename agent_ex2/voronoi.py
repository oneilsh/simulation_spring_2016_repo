...
# 2D input points.
#
# With (still buggy) modifications by Shawn O'Neil modifying
# https://code.google.com/p/geospatialpython/downloads/detail?name=voronoi.py
#
...
class Context( object ):
    ...
    ## This is still buggy because it seems in some cases the lid and rid don't always
    ## correspond to the "left" and "right" being infinite? 
    def clip_line(self,edge,lid,rid):
        ...
        # Prevent div by 0s; not the best way to handle
        # these edge cases though as the direction of the half edge may
        # not be captured
        if abs(a) < 0.000000001: a = 0.0000001
        if abs(b) < 0.000000001: b = 0.0000001
...
# given a list of points (objects with .x and .y member variables),
# returns a set of line segments (as tuples containing p1x = el[0], p1y = el[1], 
# p2x = el[2], p2y = el[3]). Some edge cases may still be buggy (e.g. colinear points
# and nearly horizontal or vertical segments). minx, miny, maxx, and maxy should describe
# a "bounding box" for the points which infinitely long segments will be trimmed to.
def voronoi_segments(points, minx, miny, maxx, maxy):
    ...
    for e in context.edges:
        ...
        v1index = e[1]
        v2index = e[2]
        if v1index >= 0 and v2index >= 0:
            p1 = context.vertices[v1index]
            p2 = context.vertices[v2index]
            segments.append( (p1[0], p1[1], p2[0], p2[1]) )
        elif v1index < 0:
            p2 = context.vertices[v2index]
            p1 = context.extra_edges[-1*v1index]
            segments.append( (p1[0], p1[1], p2[0], p2[1]) )
        else: # v2index < 0
            p1 = context.vertices[v1index]
            p2 = context.extra_edges[-1*v2index]
            segments.append( (p1[0], p1[1], p2[0], p2[1]) )
    
    return segments

    #return ( context.vertices, context.lines, context.edges, context.extra_edges, (context.xmin,context.ymin,context.xmax,context.ymax))


#------------------------------------------------------------------
def computeVoronoiDiagram( points, minx, miny, maxx, maxy ):
    """ Takes a list of point objects (which must have x and y fields).
        Returns a 3-tuple of:
    ...
    #context.set_bounds(SiteList([Site(minx, miny), Site(minx, maxy), Site(maxy, miny), Site(maxx, maxy)]))
    context.xmin = minx
    context.ymin = miny
    context.xmax = maxx
    context.ymax = maxy
